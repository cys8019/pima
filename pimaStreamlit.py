...
if uploaded_file is not None:
    ...
    if selected_features:
        ...
        if st.checkbox("Show Precision-Recall vs Threshold Curve"):
            ...
            plt.plot(threshold, precision[:-1], label="Precision", marker='.') # threshold e
        ...
        for feature in selected_features:
            if data[feature].dtype == "float64":
                value = st.slider (f"Enter value for {feature}", float (X[feature].min()),
                float(X[feature].max()))
                new_data.append(value)
            elif data[feature].dtype == "int64":
                value = st.slider (f"Enter value for {feature}", int (X[feature].min()),
